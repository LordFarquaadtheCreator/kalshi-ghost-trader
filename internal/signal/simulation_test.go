...
// TestSimulation_HistoricalMatchReplay finds a candidate match from the live
// DB that has both FlashScore points and Kalshi market ticks, then replays
// the full match in timestamp order — ticks drive prices, points drive
// signals. Asserts the model only purchases at appropriate moments:
// serving match points with edge >= 1 cent.
func TestSimulation_HistoricalMatchReplay(t *testing.T) {
	...
	e := newTestEnv(t)
	e.gen.RegisterMarkets(candidate.eventTicker, []string{candidate.homeMkt, candidate.awayMkt})

	// Build merged event stream: ticks and points sorted by timestamp.
	...
	for _, ev := range events {
		if ev.isTick {
			if ev.tick.MsgType == "ticker" && ev.tick.Price > 0 {
				e.gen.UpdatePrice(ev.tick.MarketTicker, ev.tick.Price)
			}
			continue
		}

		e.gen.OnPoints([]store.Point{ev.point})
	...
	for i, eo := range emitted {
		...
		// Match-point player must be serving
		...
		// Match-point player must be able to win the game with next point
...
// simCandidate holds replay data for a candidate match.
...
// findSimulationCandidate searches the live DB for a settled match that has
// both FlashScore points and Kalshi market ticks.
...
// TestSimulation_SyntheticFullMatch simulates a complete 2-set match with
// realistic price evolution, asserting the model purchases exactly at the
// serving match point — not before, not after.
func TestSimulation_SyntheticFullMatch(t *testing.T) {
	e := newTestEnv(t)
	e.gen.RegisterMarkets("EVT-SIM", []string{"MKT-HOME", "MKT-AWAY"})
	...
	for _, ev := range events {
		if ev.isPrice {
			e.gen.UpdatePrice(ev.price.mkt, ev.price.price)
		}
		if ev.isPoint {
			e.gen.OnPoints([]store.Point{ev.pointEvt.pt})
			orders := e.flushAndQueryOrders(t)
			if len(orders) > 0 {
				if ev.pointEvt.mp {
